...
“**Discriminative**” 和 “**Generative**” 是机器学习中两类不同的模型方法，用于解决分类或预测任务，但它们的 **建模思路和目标不同**。下面是它们的主要区别：

---

### 🔍 一句话区别：

* **Discriminative 模型**：直接学 **输入到输出的映射**（即学 $P(y \mid x)$）。
* **Generative 模型**：学 **数据的生成过程**（即学 $P(x, y)$，可以得到 $P(x \mid y)$ 和 $P(y)$）。

---

### 📚 更具体地说：

| 属性        | Discriminative 模型                             | Generative 模型                              |
| --------- | --------------------------------------------- | ------------------------------------------ |
| 学什么？      | 学 $P(y \mid x)$：类别 y 给定输入 x 的概率               | 学 $P(x \mid y)$ 和 $P(y)$：输入 x 是如何由类别 y 生成的 |
| 举例        | Logistic Regression, SVM, Random Forest, BERT | Naive Bayes, HMM, GMM, GAN, VAE            |
| 能做什么？     | 分类、回归                                         | 分类、生成样本（图像、文本）、缺失值填补、模拟数据等                 |
| 是否建模输入分布？ | ❌ 不建模 $x$ 的分布                                 | ✅ 建模 $x$ 的生成机制                             |
| 是否可以生成数据？ | ❌ 不行                                          | ✅ 可以生成新样本（图像、文本等）                          |
| 通常性能      | 对分类更精准（因为直接优化分类任务）                            | 在数据少时更鲁棒，但可能分类性能稍弱                         |
...
* 假设你知道每类（比如“垃圾邮件”或“正常邮件”）的词语出现频率。
* 可以先生成一封垃圾邮件，再看一封新邮件像不像垃圾邮件。

#### Logistic Regression（Discriminative）

* 只学会从邮件的特征直接判断它是垃圾邮件还是正常邮件。
