...
- 高置信度阈值 (Don’t add… if confidence falls below a threshold)：一个常用的策略是，模型在预测未标记数据标签时，只有当它对预测结果非常“自信”（置信度高于某个预设的阈值）时，才把这个预测的标签和对应的数据加入到 L 中。如果模型对预测结果不太确定（置信度低），就认为这个预测不可靠，不把它加入 L，从而避免引入错误标签。
