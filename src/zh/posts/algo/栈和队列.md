...
## 数据流的中位数
- 两个优先队列，一个最大堆，一个最小堆
- 最大堆 queMin：保存较小一半的数，堆顶是较小一半中的最大值。
- 最小堆 queMax：保存较大一半的数，堆顶是较大一半中的最小值。
...
class MedianFinder {
    ...
    public MedianFinder() {
        queMin = new PriorityQueue<Integer>((a, b) -> (b - a)); // 初始化最大堆
        queMax = new PriorityQueue<Integer>((a, b) -> (a - b)); // 初始化最小堆
    }
    
    public void addNum(int num) {
        if (queMin.isEmpty() || num <= queMin.peek()) { // 如果最大堆为空或者num小于等于最大堆的堆顶元素，则将num插入最大堆
            queMin.offer(num); // 将num加入最大堆
            if (queMax.size() + 1 < queMin.size()) { // 如果最大堆的大小比最小堆的大小大于1，则将最大堆的堆顶元素移到最小堆
                queMax.offer(queMin.poll()); // 将最大堆的堆顶元素移到最小堆
            }
        } else { // 如果num大于最大堆的堆顶元素，则将num插入最小堆
            queMax.offer(num); // 将num加入最小堆
            if (queMax.size() > queMin.size()) { // 如果最小堆的大小大于最大堆的大小，则将最小堆的堆顶元素移到最大堆
                queMin.offer(queMax.poll()); // 将最小堆的堆顶元素移到最大堆
            }
        }
    }
    
    public double findMedian() {
        if (queMin.size() > queMax.size()) { // 如果最大堆的大小大于最小堆的大小，则中位数是最大堆的堆顶元素 因为最大堆的堆顶元素是较小一半中的最大值，最小堆的堆顶元素是较大一半中的最小值，所以最大堆的堆顶元素是中位数
            return queMin.peek(); // 返回最大堆的堆顶元素
        }
        return (queMin.peek() + queMax.peek()) / 2.0; // 如果最大堆的大小等于最小堆的大小，则中位数是两个堆顶元素的平均值
    }
}
```