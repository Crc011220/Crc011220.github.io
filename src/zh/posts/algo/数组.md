...
public int removeElement(int[] nums, int val) {
    int slow = 0;
    for (int fast = 0; fast < nums.length; fast++) { // 如果是val，说明是要移除的元素，则只移动快指针
...
public List<List<String>> groupAnagrams(String[] strs) {
    Map<String, List<String>> map = new HashMap<>(); // key 是排序后的字符串，value 是这些字母组成的所有原始字符串组成的列表。
    for (String str : strs) {
...
public int longestConsecutive(int[] nums) {
    Set<Integer> num_set = new HashSet<>(); // 使用set因为set的查找时间复杂度为O(1)，将nums中的元素加入set
...
public int trap(int[] height) {
    if (height.length <= 2) return 0; // 如果数组长度小于等于2，则无法形成积水
    // 从两边向中间寻找最值
    ...
    while (left <= right) {
        ...
        // 假如左边更小（maxLeft < maxRight），说明：
        // 当前柱子（height[left]）右边肯定有更高的墙（maxRight ≥ maxLeft）
        // 所以这时能接的水只取决于左边的最高值 → 可以放心移动左边指针
        if (maxLeft < maxRight) {
...
- 本质上是 找第 k 小的数，就用到了 getKthElement 函数。二分跳过不可能的元素，每次将 k 减半。
```java
class Solution {
    ...
    public int getKthElement(int[] nums1, int[] nums2, int k) {
        /* 
            getKthElement(nums1, nums2, k):
                1. 如果某个数组没了，返回另一个数组的第 k 个数
                2. 如果 k == 1，直接返回 min(nums1[0], nums2[0])
                3. 否则：
                    - 取 nums1[k/2-1] 和 nums2[k/2-1]
                    - 谁小，砍掉谁前面的部分
                    - k 减掉被砍的数量，继续递归
        */

        int length1 = nums1.length, length2 = nums2.length; // 计算两个数组的长度
        int index1 = 0, index2 = 0; // 初始化两个指针 当前在数组里查找的“起点” 会随着二分跳过不可能的元素而更新缩小范围
        int kthElement = 0; // 初始化第 k 小的元素

        while (true) {
            // 如果一个数组已经全部用完了，直接返回另一个数组的第k个
            if (index1 == length1) return nums2[index2 + k - 1];
            if (index2 == length2) return nums1[index1 + k - 1];

            // 如果k == 1，说明我们要找的是最小的那个数
            if (k == 1) return Math.min(nums1[index1], nums2[index2]);

            // 正常情况：我们尝试从两个数组里各跳k/2步，比较这两个位置的元素
            int half = k / 2;

            // 防止越界，所以是 Math.min(index + half, len) - 1
            ...
            // 谁小，说明它前面那部分都不可能是第k小的元素
            if (pivot1 <= pivot2) {
                // 剔除 nums1[index1 到 newIndex1] 这段长度为 step = (newIndex1 - index1 + 1)
                k -= (newIndex1 - index1 + 1); // k缩小
                index1 = newIndex1 + 1;
            } else {
...
public int mySqrt(int x) {
    int l = 0, r = x, ans = -1; // 初始化左右指针和答案
    while (l <= r) {
        int mid = l + (r - l) / 2;
        if ((long) mid * mid <= x) { // 如果mid的平方小于等于x，则mid是x的平方根
            ans = mid;
            l = mid + 1;
        } else {
            r = mid - 1;
